...
# ENV → auto-decide profile (overridden by external AWS_PROFILE_ROTATOR if provided)
...
if [[ -n "${NEW_ID:-}" && -n "${NEW_SECRET:-}" ]]; then
  USE_AWS_API=0
  info "Use provided NEW_ID/NEW_SECRET (Proceed without invoking IAM API)"
fi

AWS_CLI=(aws --profile "$AWS_PROFILE_ROTATOR")

# (Only in API mode) Pre-validate profile
...
# Identify target IAM user / old key & create new key (Only in API mode)
OLD_KEY_ID=""
...
#    echo "      'argocd app sync ${ARGO_APP} --grpc-web' execute this."
#  }
#fi
