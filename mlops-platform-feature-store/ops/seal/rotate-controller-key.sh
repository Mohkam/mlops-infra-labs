...
# Options:
#   INCLUDE_BOOTSTRAP=1  # re-seal notifications as well
#   DRY_RUN=1            # show flow only without patch/seal/commit
# Warning:
...
RENEW_SHORT="${RENEW_SHORT:-1m}"   # temporary shortened period
...
backup_keys () {
  ...
  say "[1/6] Backup existing keys → $out"
  if [[ "$DRY_RUN" != "1" ]]; then
...
patch_args_temp_short () {
  say "[2/6] Encourage controller to add new key (temporarily shorten renew period: $RENEW_SHORT)"
  if [[ "$DRY_RUN" == "1" ]]; then
  ...
  # if args empty or renew arg missing -> add; else replace
  if yq -e '.[0]' /tmp/ss-args-backup.json >/dev/null 2>&1; then
      # array exists -> replace/add renew arg
  ...
  # Wait for new key creation: compared to existing key count +1
  local before after waited=0
  before=$(get_key_count)
  say "  Current key count: $before → waiting for new key creation..."
...
reseal_all () {
  ...
  # quick status check via argocd (if available)
...
restore_args_normal () {
  ...
  if [[ -s /tmp/ss-args-backup.json ]]; then
    # Replace/add renew argument in backup args and restore
    if yq -e '.[0]' /tmp/ss-args-backup.json >/dev/null 2>&1; then
      if yq -e 'map(select(test("^--key-renew-period="))) | length > 0' /tmp/ss-args-backup.json >/dev/null 2>&1; then
    ...
    echo "⚠️  /tmp/ss-args-backup.json missing (skip restore). Manual check recommended."
  fi
}
...
final_check () {
  ...
  say "✅ Key rotation & re-seal procedure completed"
  say "ℹ️ (optional) Cleanup old keys after all environments are healthy and several deployment cycles pass"
}
