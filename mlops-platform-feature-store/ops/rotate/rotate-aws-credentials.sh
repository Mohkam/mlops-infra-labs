...
# ===== Auto-resolve ENV to profile (AWS_PROFILE_ROTATOR from outside takes precedence) =====
: "${AWS_PROFILE_PREFIX:=rotator}"   # Change the prefix with export AWS_PROFILE_PREFIX=myrotator
if [[ -z "${AWS_PROFILE_ROTATOR:-}" ]]; then
  case "$ENV" in
    ...
    *)    echo "ERROR: unsupported ENV: $ENV (dev|prod)"; exit 1 ;;
  esac
fi
...
# 1) NEW_ID/NEW_SECRET not provided => create a new key via the IAM API (profile check required)
# 2) NEW_ID/NEW_SECRET provided     => proceed with the supplied values (profile check not required)
USE_AWS_API=1
if [[ -n "${NEW_ID:-}" && -n "${NEW_SECRET:-}" ]]; then
  USE_AWS_API=0
  info "Use provided NEW_ID/NEW_SECRET (proceed without IAM API)"
fi

AWS_CLI=(aws --profile "$AWS_PROFILE_ROTATOR")

# ===== Profile pre-check (API mode only) =====
if [[ "$USE_AWS_API" -eq 1 ]]; then
  if ! "${AWS_CLI[@]}" sts get-caller-identity >/dev/null 2>&1; then
    echo "❌ ERROR: STS check failed for AWS profile '$AWS_PROFILE_ROTATOR'"
    echo "   Prepare one of the following and try again:"
    echo "   1) aws configure --profile $AWS_PROFILE_ROTATOR (register access key/secret)"
    echo "   2) Configure the profile using your org standard approach such as SSO/Assume-Role"
    echo "   3) Or pass NEW_ID/NEW_SECRET directly as environment variables (skip IAM API)"
    exit 1
  fi
fi

# ===== Determine target IAM user / old key and create a new key (API mode only) =====
OLD_KEY_ID=""
...
if [[ "$USE_AWS_API" -eq 1 ]]; then
  ...
  if [[ "$COUNT" -ge 2 ]]; then
    echo "❌ ERROR: ${TARGET_USER} already has 2 keys. Disable or delete one and retry."; exit 1
  fi
  ...
  info "TARGET_USER not used (using already-issued credentials)"
fi

# ===== Create/update SealedSecret =====
...
#    echo "HINT: run 'argocd login <HOST> --username ... --password ... --insecure --grpc-web' first"
#    echo "      then run 'argocd app sync ${ARGO_APP} --grpc-web'."
#  }
#fi

# ===== Update local AWS credentials (optional) =====
: "${UPDATE_LOCAL:=1}"   # 1=perform update, 0=skip
PROFILE="$AWS_PROFILE_ROTATOR"      # Pin so it does not fall back to default
...
if [[ -n "${OLD_KEY_ID:-}" ]]; then
  info " ❗ After confirming the service is healthy, set the old key (${OLD_KEY_ID:0:4}********${OLD_KEY_ID: -4}) to inactive and delete it."
else
